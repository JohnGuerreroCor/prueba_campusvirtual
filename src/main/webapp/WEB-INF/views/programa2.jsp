	...
	<div id="master-content" layout="column">
        ...
        <md-content id="programa-description" class="bg-main-gray" layout="row" layout-align="center center">
			...
			<div id="imagen-programa" flex  hide-xs hide-sm>
				<img ng-src="${constantes.RUTA_ADMIN}/ofertaAcademica/imagen?id={{p.oferta.codigo}}" alt="{{p.oferta.programa.nombre}}" title="{{p.oferta.programa.nombre}}"><!-- {{p.obtenerImagen(p.oferta.imagen)}} -->
			</div>
			...
			<div id="description-programa" flex layout="column">
				...
				<img class="imagen-programa-mobile"  ng-src="${constantes.RUTA_ADMIN}/ofertaAcademica/imagen?id={{p.oferta.codigo}}" alt="{{p.oferta.programa.nombre}}" title="{{p.oferta.programa.nombre}}" flex hide-gt-sm><br><!-- {{p.obtenerImagen(p.oferta.imagen)}} -->
				<!-- More info -->
				...
				<md-button ng-if="p.cantidadInscripciones.cantidadInscritos < p.oferta.cupoMaximo || p.oferta.cupoMaximo == -1" ng-disabled="!checked" hide-sm hide-xs class="bg-main-red text-main-white b-preins" ng-click="p.editar('Adicionar')">Inscripción</md-button>
				<md-button ng-if="p.cantidadInscripciones.cantidadInscritos < p.oferta.cupoMaximo || p.oferta.cupoMaximo == -1" ng-disabled="!checked" hide-gt-sm class="bg-main-red text-main-white b-preins-mobile" ng-click="p.editar('Adicionar')">Inscripción</md-button>
