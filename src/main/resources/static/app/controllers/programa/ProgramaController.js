	...
	function ProgramaController($http, $location, $window, $mdDialog, $mdMedia, $resource, CONFIG, $sce) { //
		...
		self.loadPrograma = function() {
			$http.get(
					...
					function(data) {
						self.oferta = data;
						self.fechaActual = new Date(new Date());
						self.fechaVence = new Date(data.fechaIncripcionFin);
						
						self.fechaVence.setDate(self.fechaVence.getDate() + 1);
		...
		function DialogController($scope, $http, $mdDialog, accion, items) {
			...
			$scope.answer = function(answer) {
				...
				if (accion == "InsCerrada") {
					...
					self.registrar.usuario = $scope.usuarior; 
					self.registrar.persona = $scope.usuarior.persona; 
		...
		self.editar = function (accion) {
			...
			self.registrar.proveedores = self.oferta.internaProveedores;
			self.registrar.pw = "";
